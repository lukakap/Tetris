...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		body = points;
		initializeWidthAndHeight(points);
		fillSkirt(points);
		next = null;
	}
	...
	private void fillSkirt(TPoint[] points) {
		skirt = new int[width];
	...
	private void initializeWidthAndHeight(TPoint[] points) {
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		int widthOfRotatedPiece = computeWidthOfRotatedPiece(points);
		reflectHorizontally(points, widthOfRotatedPiece);

		Piece rotated = new Piece(points);
		return rotated;
	}


	//width of rotated piece will always be same of previous piece's height
	private int computeWidthOfRotatedPiece(TPoint[] points) {
		return height;

	}

	private void reflectHorizontally(TPoint[] points, int widthOfRotatedPiece) {
		double horizontalLine = (widthOfRotatedPiece-1)/2; //Its not used, but for visualization
		for(int i = 0; i < points.length; i++) {
			points[i].x = widthOfRotatedPiece - 1 - points[i].x;   //formula - horizontalLine + (horizontalLine - points[i].x)
		}
	}
	...
	*/
	public Piece fastRotation() {
		if(next == null) return makeFastRotations(this).next;
		return next;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece current = root;
		while(true) {
			Piece nextRotatedPiece = current.computeNextRotation();
			if (nextRotatedPiece.equals(root)){
				current.next = root;
				return root;
			}

			current.next = nextRotatedPiece;
			current = nextRotatedPiece;
		}
	}
